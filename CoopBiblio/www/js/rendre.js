                        ...
                        if(res[0]=="L"){
                            ...
                            db.scan({ TableName: 'Emprunt' }, emprunt = function(err, data) {
                                ...
                                for (var i = 0; i < data.Items.length; i++) {
                                    ...
                                    if(idL === idLivre){
                                        idMembre = parseInt(src.CodeMembre.N);
                                        existe = true;
                                        ...
                                        db.getItem({TableName: 'Membre', Key:{'Code': {'N' : data.Items[i].CodeMembre.N}}}, 
                                            function(err3, data3) 
                                            {  
   ...
   function confirmCallback(buttonIndex) {
       if(buttonIndex == 1){
            for(var i in listeRemises){
                db.deleteItem({TableName:'Emprunt',
                    Key:{
                        'CodeMembre': { 'N': listeRemises[i].idMembre.toString()},
                        'CodeLivre': { 'N': listeRemises[i].idLivre.toString()}
                    }}, function(err, data) {
                    renderBookList.empty();
                    $("#validerRendu").hide();
                });
            }
            setTimeout(function(){AfficherListe();listeRemise=[];},1000);
        }
    }  
}